...
The GEE Model uses pre-defined clusters to introduce the possibility that the independent variables (COVID risk) may be correlated between counties. The Kulldorff clusters identified by SaTScan and SpatialEpi present likely groups of counties with correlated COVID risk. Chakraborty combined these Kulldorff clusters with state codes to create the predefined GEE clusters. Thus, the GEE model is accounting for potential correlation between counties which are 1. in the same state, and thus may have experienced similar COVID responses, and 2. have been identified (either by SaTScan or SpatialEpi) to have similar relative COVID risk. 

This means that any given state may have one or more GEE clusters within it, but that Kulldorf clusters previously crossing state lines are divided into multiple clusters. Furthermore, if a state has multiple clusters with the same relative risk, those clusters would be combined into one spatially discontinuous cluster. This is visualized here:
...
This is code that i haven't looked at yet
# count frequencies of each cluster type
clus_counts <- satscan_rpr_col %>%
  st_drop_geometry() %>%
  group_by(GINI_CLUST) %>%
...
#PICK UP HERE, MAKE SURE TO ADJUST COLORS; WONT NEED ALL THREE BLOCKS
...
## LOAD MAP OF AUTHOR GENERATED KULLDORF CLUSTERS
# count frequencies of each cluster type
clus_counts <- satscan_rpr_col %>%
  st_drop_geometry() %>%
  group_by(GINI_CLUST) %>%
