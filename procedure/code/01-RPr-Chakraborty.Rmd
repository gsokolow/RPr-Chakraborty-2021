...
author: "Joseph Holler, Junyi Zhou, Peter Kedron, Drew An-Pham, Derrick Burt, Grace Sokolow"
date: "`r Sys.Date()`"
...
## interesting - I'm getting a data frame with 0 features. Did the original data source get modified?
...
###How does the GEE Model use Kulldorff Clusters?
The GEE Model uses pre-defined clusters to introduce the possibility that the independent variables (COVID risk) may be correlated between counties.
The Kulldorff clusters identified by SaTScan and SpatialEpi present likely groups of counties with correlated COVID risk.
Chakraborty combined these Kulldorff clusters with state codes to create the predefined GEE clusters.
Thus, the GEE model is accounting for potential correlation between counties which are 1.
in the same state, and thus may have experienced similar COVID responses, and 2.
have been identified (either by SaTScan or SpatialEpi) to have similar relative COVID risk.

This means that any given state may have one or more GEE clusters within it, but that Kulldorf clusters previously crossing state lines are divided into multiple clusters.
Furthermore, if a state has multiple clusters with the same relative risk, those clusters would be combined into one spatially discontinuous cluster.
This is visualized here:

```{r create-sample-gee_dataset}
## Create sample region dataset
#Load GEE data
# Filter by State FP to create our sample region
# from https://www.mercercountypa.gov/dps/state_fips_code_listing.htm
  # California: 06
  # Nevada: 32
  # Arizona: 04

gee_example <- gee_data %>%
  filter(statefp == '06'  | statefp == '04' | statefp == '32') %>%
# add column for each gee cluster
  mutate(sc_id = paste(statefp,cluster_class))
  


#Visualize filtered GEE data and variables of interest: cluster_class (relative risk), cluster id (unique cluster identifier), isCluster (whether the county is the center of the cluster)
gee_example %>%
  st_drop_geometry %>%
  arrange(fips) %>%
  select(statefp, cluster_class, sc_id, clusterID, isCluster) %>%
  kable(caption = "GEE Cluster IDs, Fips Codes, & Relative Risk for sample region") %>% kable_styling()
```

Now that we've established our study region, we can visualize it. First, we filter the states data set so that only our study region appears on the map. To make it easier to visualize, we filter for an example of each type of Kulldorff - GEE cluster relationship: 
  1. A Kulldorff cluster that crosses state lines is divided into one cluster per state.
  2. Multiple Kulldorff clusters in the same state with the same relative risk are combined, even if they are spatially discontinuous.
  3. A single Kulldorff cluster is the sole cluster at a given relative risk level in a given state and thus does not change.

```{r filter--GEE-sample-clusters}

#Filter states dataset by study region
study_state <- filter(state, STATEFP == '06'  | STATEFP == '04' | STATEFP == '32')
# filter example data for target clusters:
# 062 and 322 show a cluster getting separated by state line
# 041 shows discontinuous clusters being joined
# 045 shows a cluster that doesnt change
study_gee <- filter (gee_example, sc_id == '06 2' | sc_id == '32 2' | sc_id == '04 1' | sc_id == '04 5')

# Manually enter the legend labels (this would be a spot for more elegant code)
study_labels <- c('AZ 1', 'AZ 5', 'CA 2', 'NV 2')

```

First, we will map the Kulldorff clusters generated by SaTScan before they are combined with states, for reference:

Note that the SaTScan generated Kulldorff clusters did not include all counties in a cluster, but those generated by SpatialEpi did. This is of particular relevance in Arizona, where some counties in clusters are not shown in the first map. Both sets of clusters are combined with states to be used in the GEE Model. A future revision to this study would make two sets of the following maps: one for each data type. Further work could also be done to better align the color schemes of the two maps.

```{r map-kulldorf-no-state}
#create the map
tm_sample_clusters_1 <- 
  tm_shape(study_state) +
    tm_fill("gray") +
tm_shape(study_gee) +
tm_polygons(col = char("clusterID"),
            labels = study_state$clusterID,
            border.col = "white",
            #line width
            lwd = 0.5,
            ...
            #popup.vars = c("fips", "clusterID"),
            colorNA = NULL,
            title = "Sample GEE Clusters in the Southwestern US")

  #visualize
tm_sample_clusters_1

```
Second, we map the Kulldorff clusters once they have been combined with state ids. This is what the GEE model uses. 

The main thing to note between these two maps is that the class 2 risk cluster in California and Nevada is divided into two distinct clusters. However, in California and in Arizona, there are discontinuous clusters of the same risk class that get grouped together. Arizona also has a cluster that does not cross state lines and has a different relative risk than the rest of the state; this cluster remains unchanged.

```{r map-gee-sample-clusters}

#create the map
tm_sample_clusters_2 <- 
  tm_shape(study_state) +
    tm_fill("gray") +
tm_shape(study_gee) +
tm_polygons(col = char("sc_id"),
            labels = study_labels,
            border.col = "white",
            #line width
            lwd = 0.5,
...
This operationalization excluded all but the center county of each cluster and assigned the other counties to the lowest risk category (Figure 4).
...
We proceeded to combine the local COVID risk score with State IDs for use as the GEE clustering ID and to run the GEE models.
...
The chloropleth map we made in the **first part** of the reproduction analysis (Figure 2) reveals an identical spatial pattern to that of the original study's Figure 1, confirming the equivalence of our dependent variable with that of the original study.
...
The spatial dependence between counties in different clusters is not modeled, even if the counties are adjacent to each other.
...
Our reproduction study was partially successful.
With assistance from the author and use of some free, but not open source software, we were able to reproduce the results of the original study.
However, in the course of doing so, we discovered that the methods used in the original study, particularly in creating the Kulldorff clusters, included implicit choices in research design that were not originally stated, or indeed, contemplated, in the original study.
While we were able to both reproduce the original results and conduct slightly altered analyses, this reproduction study emphasizes that all of the choices made in a study, including those implicit in software selection and the use of default parameters, impact how appropriately the results can be interpreted.
